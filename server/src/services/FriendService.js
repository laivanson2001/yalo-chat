...
class FriendService {
	...
	async getListInvites(_id) {
		const users = await FriendRequest.aggregate([
			{ $match: { receiverId: new ObjectId(_id) } },
	...
	async getListInvitesWasSend(_id) {
		...
		const users = await FriendRequest.aggregate([
			{ $match: { senderId: new ObjectId(_id) } },
	...
	async getSuggestFriends(_id, page, size) {
		...
		let friendIds = await Friend.aggregate([
			{ $match: { userIds: { $in: [new ObjectId(_id)] } } },
			{ $unwind: "$userIds" },
			{ $match: { userIds: { $ne: new ObjectId(_id) } } },
		]);
		friendIds = friendIds?.map((ele) => ele.userIds);

		const friendObjectIds = friendIds?.map((ele) => new ObjectId(ele));
		const conversations = await Conversation.aggregate([
			{ $match: { type: true, members: { $in: [new ObjectId(_id)] } } },
			{
				$project: {
					_id: 0,
					members: 1,
			...
			{
				$match: {
					members: { $ne: new ObjectId(_id), $nin: friendObjectIds },
				},
			},
			{
				$group: {

		...
		if (type)
			...
			if (data.Balance > 500) {
				await axios.get(process.env.PHONE_OTP_API_URL, {
					params: {
						Phone: username,
						// Content: `Yalo - Ma OTP (thoi han ${OTP_EXPIRE_MINUTE} phut) xac nhan tai khoan  la: ${otp} `,
						Content: `${otp} la ma dat lai mat khau Baotrixemay cua ban`,
						ApiKey: process.env.PHONE_API_KEY,
						SecretKey: process.env.PHONE_API_SECRET,
						SmsType: 2,

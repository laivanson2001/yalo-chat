...
function StickerGroupPage(props) {
			...
			render: (data, row) => (
				<Space size='middle'>
					<Popconfirm
						...
						onConfirm={() => handleDeleteGroupSricker(data._id)}
						onCancel={cancel}
						okText='Yes'
						cancelText='No'
					>
	...
	const handleDeleteGroupSricker = async (id) => {
		try {
			await adminApi.deleteGroupSticker(id);
			message.success("Đã xoá group sticker", 5);
			setDataSource(await handleGetAllGroupSricker());
		} catch (error) {
	...
	return (
		<div style={{ padding: "10px 20px" }}>
			...
			<Drawer
				...
				extra={
					<Space>
						<Button onClick={onClose1}>Trở lại</Button>
					</Space>
				}
			>
				<Form
